...
| UC-24 | 文体をフレンドリーへ変更して送信 | 応答文体切替 | 応答がフレンドリー文体になる | `responseTone=friendly` が送信される |
| UC-25 | アプリ再起動後に文体設定確認 | 設定保持 | 直前の文体設定が保持される | `localStorage` から設定復元される |
...
1. 設定UIで `丁寧/フレンドリー/簡潔` を選択
2. 選択値を `localStorage` に保存
3. 送信時に `responseTone` をAPIへ送信
4. APIは指定文体で応答文を返す
...
- 文体は `polite/friendly/concise` を切り替え可能
- 文面候補は決定論ローテーションで選択する
