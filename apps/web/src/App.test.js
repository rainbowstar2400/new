...
describe("App", () => {
    ...
    it("disables text input during choice_only confirmation", async () => {
        ...
        render(_jsx(App, {}));
        await waitFor(() => expect(screen.getByRole("heading", { name: "自分専用秘書PWA" })).toBeInTheDocument());
        const textarea = screen.getByPlaceholderText("例: 明日9時にAさんへ連絡");
        ...
        const lockedTextarea = screen.getByPlaceholderText("例: 明日9時にAさんへ連絡");
    ...
    it("enables text input after negative choice in choice_then_text_on_negative", async () => {
        ...
        await waitFor(() => {
            const unlockedTextarea = screen.getByPlaceholderText("例: 明日9時にAさんへ連絡");
    ...
    it("stores response tone and sends it in chat payload", async () => {
        ...
        vi.stubGlobal("fetch", vi.fn(async (input, init) => {
            ...
            if (url.includes("/v1/chat/messages")) {
                lastChatPayload = JSON.parse(String(init?.body ?? "{}"));
                return new Response(JSON.stringify({
        ...
        render(_jsx(App, {}));
        await waitFor(() => expect(screen.getByRole("heading", { name: "自分専用秘書PWA" })).toBeInTheDocument());
        const toneSelect = screen.getByLabelText("応答文の文体");
        ...
        if (lastChatPayload === null)
            throw new Error("chat payload is null");
