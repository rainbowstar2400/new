...
import type { ChatMessageResponse } from "@new/shared";
import { chatControlHint, inferChatControl, isTextInputAllowed } from "./chat-control";

function baseResponse(overrides: Partial<ChatMessageResponse>): ChatMessageResponse {
  return {
    assistantText: "ok",
    summarySlot: "ok",
    actionType: "confirm",
...
describe("chat-control", () => {
  it("uses explicit API meta when provided", () => {
    const control = inferChatControl(
      baseResponse({
